        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var shoppingItems = await _db.ShoppingCarts.Where(s => s.ApplicationUser.Email == model.Email).CountAsync();

                    //var user = await _db.Users.SingleOrDefaultAsync(u => u.Email == model.Email);

                    //var shoppingItems = await _db.ShoppingCarts.Where(s => s.ApplicationUserid == user.Id).CountAsync();
